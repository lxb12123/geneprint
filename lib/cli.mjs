...
export function inherit(targetDir, { name, from, target }) {
  ...
  if (target === 'plugin') result.plugin = pack(targetDir);      // 顺手打包成可安装插件
  return result;
}
...
if (isMain()) {
  ...
  if (cmd === 'inherit') {
    ...
    console.log(JSON.stringify(pack(targetDir)));   // 基因项目 → 可安装的跨平台插件
    ...
    console.error('usage: agent-path-forge <inherit|pack|scaffold|eval|trace|list> <dir> [--name <name>] [--from <skillDir|registry-name>] [--target plugin] [--runs <runsFile>]');
    process.exit(1);
  }
}
